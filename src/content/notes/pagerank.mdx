---
title: "Markov Chains and Principal Eigenvectors in Network Centrality: PageRank"
description: "A mathematical derivation of node-level feature engineering in GNNs, focusing on the spectral properties of Markov Chains and the convergence of the PageRank principal eigenvector."
pubDate: 2025-10-03
tags: [""]
---
...
<Theorem title="PageRank and the Flow Equation">
The **PageRank** vector is defined as the principal eigenvector of the transition matrix $M$. 

If $r$ is the limit of the power sequence $M^k u$ as $k \to \infty$, then $r$ satisfies the **flow equation**:
$$
r = M r
$$
This confirms that $r$ is the principal eigenvector of $M$ associated with the eigenvalue $1$. To efficiently solve for $r$ in high-dimensional systems (like the web graph), we typically employ the **Power Iteration Method**.
</Theorem>
...
<Theorem title="Spectral Properties of Markov Matrices">
A **Markov matrix** $A$ always has an eigenvalue $\lambda = 1$. All other eigenvalues $\lambda_i$ satisfy:
$$
|\lambda_i| \le 1
$$
...
<Theorem title="Perron’s Theorem for Positive Matrices">
If $A_{n \times n} > 0$ with $r = \rho(A)$, then:

1. $r > 0$.
2. $r \in \sigma(A)$ (the *Perron root*).
3. $\text{alg mult}_A(r) = 1$ (simple root).
4. There exists an eigenvector $x > 0$ such that $Ax = rx$.
5. The Perron vector $p$ is unique:
   $$ Ap = rp, \quad p > 0, \quad \|p\|_1 = 1 $$
6. $r$ is the only eigenvalue on the spectral circle.
7. **Collatz–Wielandt formula**:
   $$ r = \max_{x \in N} \min_{\substack{1 \leq i \leq n \\ x_i \neq 0}} \frac{[Ax]_i}{x_i} $$
</Theorem>
...
<Theorem title="Perron’s Theorem for Nonnegative Matrices">
If $A_{n \times n} \geq 0$ with $r = \rho(A)$, the following statements are true:

*   $r \in \sigma(A)$ (note that $r = 0$ is possible).
...
<Theorem title="Statement: Irreducibility">
A square matrix $A$ is **irreducible** if and only if its directed graph is strongly connected. 

Equivalently, for each pair of indices $i, j$ there exist $t \ge 1$ and intermediate indices $k_1, \dots, k_{t-1}$ such that:
$$
...
<Theorem title="Perron–Frobenius Theorem">
Let $A \in \mathbb{R}^{n \times n}$ be an **irreducible** matrix with $A \ge 0$. Then:  

1. $r = \rho(A) > 0$.  
2. $r \in \sigma(A)$ (the Perron root).  
3. $\operatorname{algmult}_A(r) = 1$ (the Perron root is simple).  
4. There exists an eigenvector $x > 0$ such that:
   $$ Ax = rx $$
5. The Perron vector is the unique vector defined by:
   $$ Ap = rp, \quad p > 0, \quad \|p\|_1 = 1 $$
   Except for positive multiples of $p$, there are no other nonnegative eigenvectors of $A$, regardless of the eigenvalue.
