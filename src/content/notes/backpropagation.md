---
title: "Stochastic Gradient Descent & The Error Backpropagation Calculus"
description: "An analytical derivation of the backpropagation algorithm, examining the application of the multivariable chain rule to gradient-based optimization. This essay bridges the gap between the calculus of sensitivity analysis and its high-performance implementation in deep learning architectures."
pubDate: 2025-01-08
tags: [""]
---
...
![](./backpropagation/activation.svg)
...
\tag{4}
$$
What backpropagation allow us to do is to calculate $\frac{\partial C_{X}}{\partial w}$ and $\frac{\partial C_{x}}{\partial b}$ and then finding $\frac{\partial C}{\partial w}$ and $\frac{\partial C}{\partial b}$ averaging over the training samples.
...
Turning back to our final objective, that is to compute $\frac{\partial C}{\partial w^l}$.
...
\end{pmatrix}
$$
What is $\frac{\partial C}{\partial W}$?
...
$$
\frac{\partial C \left(y(x) \right)}{\partial x}
$$
Where:
- $C$ is a scalar function
- $y:\mathbb{R}^n\to \mathbb{R}^m$ is a vector valued function
- and $x \in \mathbb{R}^n$
The composition $C(y(x))$ is still a scalar function — it maps $x∈\mathbb{R}^n$ to a real number via $y$.
...
But now, we can go a step further and express how $C$ depends on $x$ **through** $y(x)$.
$$
C=C(y_{1}(x),y_{2}(x),\dots,y_{m}(x))
$$
...
$$
\frac{\partial C}{\partial x_{j}}=\sum_{i=1}^m \frac{\partial C}{\partial y_{i}} \cdot\frac{\partial y_{i}}{\partial x_{j}}
$$
This tells us: to compute how $C$ changes as we vary $x_{j}$​, we look at how each $y_{i}$​ changes with $x_{j}$​, weighted by how sensitive $C$ is to $y_{i}$​.

$x$ is a vector of $n$ components, so if we stack all these pieces of chain rule together we get:
$$
...
- The **Jacobian matrix** $\frac{\partial y}{\partial x}\in \mathbb{R}^{m \times n}$ , whose $(i,j)-th$ entry is $\frac{\partial y_{i}}{\partial x_{j}}$
- The **gradient of $C$ with respect to $y$**, $\nabla_{y} C =\mathbb{R}^m$, whose $i$ components is $\frac{\partial C}{\partial y_{i}}$
...
\end{pmatrix}
$$
Our case the scalar C depends on a vector $y$, and that vector depends on the matrix $W$

-  $C: \mathbb{R}^{m} \to \mathbb{R}$  is a scalar function  
-  $y: \mathbb{R}^{k \times n} \to \mathbb{R}^{m}$ is a vector-valued function  
- $W \in \mathbb{R}^{k \times n}$ is a matrix input

---

In our case the scalar function $C$ Depends on a vector $y$ and the vector depends on a matrix $W$.

This leads us to the composition

$$
C\left(y\left(W\right)\right)
$$
With $y$ with the form of:
$$
y=\begin{pmatrix}
y_{1}(W) \\
y_{2}(W) \\
...
\end{pmatrix}
$$
Now we have to use the matrix definition looking at the components of the matrix $W$:
$$
\left[ \frac{\partial C\left(y(W)\right)}{\partial W}\right]_{jk} = \frac{\partial C\left(y(W)\right)}{\partial w_{jk}}
$$

How $C$ depends on $w_{ij}$? It depends on $y$ that depends on $W$. So let's apply the chain rule but for scalars.
$$
\frac{\partial C(y(W))}{\partial w_{jk}} = \sum_{i=1}^m \frac{\partial C}{\partial y_i} \cdot \frac{\partial y_i}{\partial w_{jk}}
$$
...
- $\frac{\partial C}{\partial y}$ it's a vector
So we can write what we wrote as:
$$
\frac{\partial C(y(W))}{\partial W} = \sum_{i=1}^k \frac{\partial C}{\partial y_{i}} \cdot \frac{\partial y_{i}}{\partial W}
$$
...
$$
\frac{\partial C}{\partial W} = \sum_{i=1}^k \frac{\partial C}{\partial y_{i}} \cdot \frac{\partial y_{i}}{\partial W} \to \frac{\partial C}{\partial W} =  \frac{\partial y}{\partial W} \overset{i}{\cdot} \frac{C}{\partial y}
...
\tag{7}
$$
We are taking the derivative with respect to a matrix, but this scalar depends on a vector, that depends on a vector that depends on a vector and so on until it depends on the matrix $w^l$.

We start from the outermost layer, the layer linked with the matrix $w^l$ and we compute the tensor contraction:
$$
\frac{\partial C}{\partial W} =  \frac{\partial a^l}{\partial w^l} \overset{i}{\cdot} \frac{C}{\partial a^l}

\tag{8}
$$
But the vector $a^l$ depends on the vector $a^{l+1}$ and so on until the last layer $a^L$, to compute how each layer change each other we use the chain rule for vectors we found earlier:
...
$$
And that is the recursive hear of backpropagation until the last layer $a^L$.
...
$$
\frac{\partial C}{\partial a^l} = \left(\frac{\partial a^{l+1}}{\partial a^l}\right)^{\top} \cdot \frac{\partial C}{\partial a^{l+1}}  \to \delta^l= \left(\frac{\partial a^{l+1}}{\partial a^l}\right)^{\top} \cdot \delta^{l+1}
$$
...
If we are for example in the neuron $j^{th}$ in the layer one. If we make a little change to the weighed input of the neuron $z_{j}^1$ adding the quantity $\Delta z_{j}^1$, then the activation input instead of outputting $\sigma(z_{j}^1)$ it will output $\sigma(z_{j}^1 + \Delta z_{j}^1 )$.
...
$$
If we want to try to minimize the cost function, we can try to chose the quantity $\Delta z_{j}^1$ so that makes the cost smaller.
If the quantity $\frac{\partial C}{\partial z_{j}^1}$ is close to zero then there is little to change, indeed if the quantity $\frac{\partial C}{\partial z_{j}^1}$ is big we can chose the quantity $\Delta z_{j}^1$ of the opposite sign to try to minimize the cost.

In this heuristic sense, this quantity $\frac{\partial C}{\partial z_{j}^1}$ is the measure of the error of the neuron $j^{th}$ in the layer 1.
$$
\delta_{j}^1=\frac{\partial C}{\partial z_{j}^1}
$$

## Why both definitions can coexist
There is no contradiction between defining the error as $\frac{\partial C}{\partial a^l}$ or $\frac{\partial C}{\partial z^l}$, because they are directly related through the chain rule.
Since $a^l = \sigma(z^l)$, we apply the chain rule:

$$
\frac{\partial C}{\partial z^l} = \frac{\partial C}{\partial a^l} \cdot \frac{\partial a^l}{\partial z^l}
$$

Because $\sigma$ is applied elementwise, this becomes an elementwise (Hadamard) product:
...
So depending on the context, one may define and propagate $\tilde{\delta}^l$ (via the recursive expression), and then obtain the standard error  $\delta^l$ by applying the derivative of the activation. They are just two steps in the same computation, not conflicting definitions.
...
$$
\frac{\partial C}{\partial w^l} \text{ for all layers  }l
$$
Once we know the vector $\delta^l$ we can compute the weight gradient using a tensor contraction (using equation 8).
$$
...
By the definition of gradient follows that if we want to minimize our scalar function, that quantifies how far the model’s prediction $h(x)$ is from the true output $y$ for a **single data point**, then we have to move in the opposite direction of the gradient from that point $p$.

The update rule for the gradient descend is:
$$
a_{n+1} = a_{n} - \eta\nabla f(a_{n})
$$
$\eta$ is the learning rate, is represent how long is the step you are going to take towards the function local minimum, $a_{n}$ is the point you are evaluating the gradient, from that point you move to the direction of the fastest decrease of the function.

How can we apply gradient descent to learn in a neural network? The idea is to use gradient descent to find the weights $w_{k}$ and biases $b_{l}$ which minimize the cost in Equation. To see how this works, let's restate the gradient descent update rule, with the weights and biases replacing the variables $v_{j}$. In other words, our "position" now has components $w_{k}$ and $b_{l}$, and the gradient vector $\nabla C$ has corresponding components $\frac{\partial C}{\partial w_{k}}$ and $\frac{\partial C}{\partial b}$. Writing out the gradient descent update rule in terms of components, we have the update rule for the bias:
$$ 
...
We are calling those random input training samples $(X_{1},X_{2},\dots,X_{m} )$ we will call them **mini-batch**.
...
### 2) phase
We need to find the quantities $\frac{\partial C}{\partial w^l}$ and $\frac{\partial C}{\partial b^l}$ for all layers $l$.
...
$$
\sigma(x)=\frac{1}{1+e^{-x}},\quad \text{with} \quad \sigma^{′}(x)=\sigma(x)(1-\sigma(x))
$$
...
We start computing $\delta^L$ the error for the last layer we defined as:
$$
\delta^L :=\frac{\partial C}{\partial a^L}
$$

Now if we take the derivative of the squared error with respect to $a^L$ we obtain (check for yourself):
$$ 
...
because this is a valid partial derivative that describes how the cost changes with respect to the activation at layer $L$. However, during backpropagation, we ultimately need the derivative with respect to $z^L$, since $z^L$depends directly on the weights. By applying the chain rule, we refine our expression:

$$
\delta^L = \frac{\partial C}{\partial z^L} = \frac{\partial C}{\partial a^L} \odot \frac{\partial a^L}{\partial z^L}.
$$
Since we now that  $\frac{\partial C}{\partial a^L} = a^L - y$, and we know the form of the derivative of the activation function:
...
Since this is a linear function of $a^l$, taking it's derivative the second term of the last expression becomes:
$$
...
So the derivative is a **diagonal matrix** with the derivative of $\sigma$ applied to each entry:
$$
\frac{\partial \sigma(z^{l+1})}{\partial z^{l+1}} = \mathrm{diag}(\sigma'(z^{l+1}))
$$

This matrix has $\sigma'(z_i)$ on the diagonal and zeros elsewhere.
...
We have not finished yet, we have to multiply this by the error of $\delta^{l+1}=\frac{\partial C}{\partial a^{l+1}}$. 
...
Once we have the error vector $\delta^l$, we can compute the gradient of the cost with respect to the weights:
...
This is the outer product between the error vector at layer $l$ and the activations from the previous layer.

###  What about the bias $b^l$?
...
This means the bias $b^l$ directly and linearly affects the pre-activation $z^l$.  
That is:
$$
...
-->[("Simon", 15), ("Erik",22)]
```
For the biases we are creating a matrix of dimentions $y \times 1$, we are not assigning it a simple vector because with a matrix will be simpler doing later operation, like calculating the activation of a neuron. 
...
If for example our `sizes` turple is `[1,2,1]` we would have two matrixes of dimentions: $2 \times 1$ and $1 \times 2$:
$$
\begin{pmatrix}
w_{1,1}^1 \\
w_{2,1}^1
...
Remember from before how we defined the activation for a layer $\ell$.
...
Remember also how we defined the middle quantity $ z$:
$$
 z^{\ell} = W^{\ell}a^{\ell-1} + b^{\ell}
$$
